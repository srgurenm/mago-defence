...
class Juego:
    def __init__(self):
        ...
        btn_size = 90
        btn_y = ALTO - btn_size - 20
        spacing = 15

        self.rect_btn_izq = pygame.Rect(20, btn_y, btn_size, btn_size)
        self.rect_btn_der = pygame.Rect(20 + btn_size + spacing, btn_y, btn_size, btn_size)

        disparo_size = 120
        self.rect_btn_disparo = pygame.Rect(ANCHO - disparo_size - 20, ALTO - disparo_size - 20, disparo_size, disparo_size)
    ...
    def detectar_dispositivo_tactil(self):
        if sys.platform == 'emscripten':
            try:
                import js
                is_touch = js.navigator.maxTouchPoints > 0 or 'ontouchstart' in js.window
                return is_touch
            except:
                return False
        return False
    ...
    def dibujar_botones_tactiles(self):
        ...
        pygame.draw.rect(s, (255, 255, 255, ALPHA_BOTONES), self.rect_btn_izq, border_radius=18)
        pygame.draw.rect(s, (255, 255, 255, ALPHA_BOTONES), self.rect_btn_der, border_radius=18)
        pygame.draw.circle(s, (0, 255, 255, ALPHA_BOTONES), self.rect_btn_disparo.center, 65)
        self.pantalla.blit(s, (0, 0))
    ...
    def update(self):
        ...
        if self.estado == ESTADO_JUGANDO:
            ...
            if teclas[pygame.K_SPACE]: self.mago.disparar(self.monstruos) 
                ...
                for tid in list(self.toques_activos.keys()):
                    if tid in self.toques_activos:
                        ...
                        if tx is None or ty is None:
                            continue
                        if self.rect_btn_izq.collidepoint(tx, ty):
                            m_izq = True
                        if self.rect_btn_der.collidepoint(tx, ty):
                            m_der = True
                        if self.rect_btn_disparo.collidepoint(tx, ty):
                            disparando = True
